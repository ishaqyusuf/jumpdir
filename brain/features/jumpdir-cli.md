...
- `jumpdir update` checks whether a newer published version or a changed same-version build is available.
- `jumpdir history` (also `jumpdir h` and `jumpdir -h`) lists recent unique action commands in alphabetical order, with `-f`/`--filter` and `-c`/`--count` options; zsh-integrated Enter or Tab pastes into the prompt without running, legacy Enter replays immediately, and Backspace deletes the highlighted entry. Help uses `--help`.
