...
- `jumpdir history` lists or interactively replays recent unique action commands, with `-f`/`--filter` and `-c`/`--count` options; Backspace deletes the highlighted entry.
